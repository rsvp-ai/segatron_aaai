...
class  PreTrainedTokenizer(object):
    ...
    def _from_pretrained(cls, pretrained_model_name_or_path, *init_inputs, **kwargs):
        ...
        if added_tokens_file is not None:
            ...
            tokenizer.unique_added_tokens_encoder = set(tokenizer.added_tokens_encoder.keys()).union(
    ...
    def encode_plus(self,
                    ...
                    text_para_pos=None,
                    text_token_pos=None,
                    text_pair_sent_pos=None,
                    text_pair_para_pos=None,
                    text_pair_token_pos=None,
                    add_special_tokens=True,
                    max_length=None,
                    stride=0,
                    truncation_strategy='longest_first',
        ...
        return self.prepare_for_model(first_ids,
                                      pair_ids=second_ids,
                                      text_sent_pos=text_sent_pos,
                                      text_para_pos=text_para_pos,
                                      text_token_pos=text_token_pos,
                                      text_pair_sent_pos=text_pair_sent_pos,
                                      text_pair_para_pos=text_pair_para_pos,
                                      text_pair_token_pos=text_pair_token_pos,
                                      max_length=max_length,
                                      pad_to_max_length=pad_to_max_length,
                                      add_special_tokens=add_special_tokens,
                                      stride=stride,
    ...
    def prepare_for_model(self, ids, pair_ids=None,
                          text_sent_pos=None,
                          text_para_pos=None,
                          text_token_pos=None,
                          text_pair_sent_pos=None,
                          text_pair_para_pos=None,
                          text_pair_token_pos=None,
                          max_length=None, add_special_tokens=True, stride=0,
                          truncation_strategy='longest_first',
                          pad_to_max_length=False,
                          return_tensors=None,
                          return_token_type_ids=True,
        ...
        Args:
            ...
            stride: window stride for overflowing tokens. Can be useful for edge effect removal when using sequential
                list of inputs.
            prepare_for_model: string selected in the following options:
                - 'longest_first' (default) Iteratively reduce the inputs sequence until the input is under max_length
                    starting from the longest one at each token (when there is a pair of input sequences)
                - 'only_first': Only truncate the first sequence
                - 'only_second': Only truncate the second sequence
        ...
        if text_sent_pos:
            assert len(text_sent_pos) == len_ids
        if text_para_pos:
            assert len(text_para_pos) == len_ids
        if text_token_pos:
            assert len(text_token_pos) == len_ids
        encoded_inputs = {}

        # Handle max sequence length
        total_len = len_ids + len_pair_ids + (self.num_added_tokens(pair=pair) if add_special_tokens else 0)
        if text_pair_sent_pos:
            assert len(text_pair_sent_pos)==len_pair_ids
        if text_pair_para_pos:
            assert len(text_pair_para_pos) == len_pair_ids
        if text_pair_token_pos:
            assert len(text_pair_token_pos) == len_pair_ids
        ...
        if max_length and total_len > max_length:
            ids, pair_ids, overflowing_tokens = self.truncate_sequences(ids, pair_ids=pair_ids,
                                                                        ...
                                                                        stride=stride)
            if text_sent_pos:
                text_sent_pos, text_pair_sent_pos,overflowing_sent_pos = self.truncate_sequences(text_sent_pos, pair_ids=text_pair_sent_pos,
                                                                            num_tokens_to_remove=total_len-max_length,
                                                                            truncation_strategy=truncation_strategy,
                                                                            stride=stride)
            if text_para_pos:
                text_para_pos, text_pair_para_pos,overflowing_para_pos = self.truncate_sequences(text_para_pos, pair_ids=text_pair_para_pos,
                                                                            num_tokens_to_remove=total_len-max_length,
                                                                            truncation_strategy=truncation_strategy,
                                                                            stride=stride)
            if text_token_pos:
                text_token_pos, text_pair_token_pos,overflowing_token_pos = self.truncate_sequences(text_token_pos, pair_ids=text_pair_token_pos,
                                                                            num_tokens_to_remove=total_len-max_length,
                                                                            truncation_strategy=truncation_strategy,
                                                                            stride=stride)
            if return_overflowing_tokens:
                ...
                if text_sent_pos:
                    encoded_inputs["overflowing_sent_pos"] = overflowing_sent_pos
                if text_para_pos:
                    encoded_inputs["overflowing_para_pos"] = overflowing_para_pos
                if text_token_pos:
                    encoded_inputs["overflowing_token_pos"] = overflowing_token_pos
        ...
        if add_special_tokens:
            ...
            if text_sent_pos:
                para_pos_sequence, sent_pos_sequence, token_pos_sequence = self.build_pos_with_special_tokens(text_para_pos,
                                                                                                              text_sent_pos,
                                                                                                              text_token_pos,
                                                                                                              text_pair_para_pos,
                                                                                                              text_pair_sent_pos,
            ...
            if text_sent_pos:
                sent_pos_sequence = text_sent_pos+text_pair_sent_pos if text_pair_sent_pos else text_sent_pos
                para_pos_sequence = text_para_pos+text_pair_para_pos if text_pair_para_pos else text_para_pos
                token_pos_sequence = text_token_pos+text_pair_token_pos if text_pair_token_pos else text_token_pos
        ...
        if text_sent_pos:
            encoded_inputs["sent_pos"] = sent_pos_sequence
            encoded_inputs["para_pos"] = para_pos_sequence
            encoded_inputs["token_pos"] = token_pos_sequence
        if max_length and len(encoded_inputs["input_ids"]) > max_length:
            ...
            if text_sent_pos:
                encoded_inputs["sent_pos"] = encoded_inputs["sent_pos"][:max_length]
                encoded_inputs["para_pos"] = encoded_inputs["para_pos"][:max_length]
                encoded_inputs["token_pos"] = encoded_inputs["token_pos"][:max_length]
        ...
        if needs_to_be_padded:
            ...
            if self.padding_side == 'right':
                ...
                if text_sent_pos:
                    encoded_inputs["sent_pos"] = encoded_inputs["sent_pos"] + [self.pad_token_id] * difference
                    encoded_inputs["para_pos"] = encoded_inputs["para_pos"] + [self.pad_token_id] * difference
                    encoded_inputs["token_pos"] = encoded_inputs["token_pos"] + [self.pad_token_id] * difference
                ...
                if text_sent_pos:
                    encoded_inputs["sent_pos"] = [self.pad_token_id] * difference + encoded_inputs["sent_pos"]
                    encoded_inputs["para_pos"] = [self.pad_token_id] * difference + encoded_inputs["para_pos"]
                    encoded_inputs["token_pos"] = [self.pad_token_id] * difference + encoded_inputs["token_pos"]
        ...
        if return_tensors == 'tf' and is_tf_available():
            ...
            if "sent_pos" in encoded_inputs:
                encoded_inputs["sent_pos"] = tf.constant([encoded_inputs["sent_pos"]])
                encoded_inputs["para_pos"] = tf.constant([encoded_inputs["para_pos"]])
                encoded_inputs["token_pos"] = tf.constant([encoded_inputs["token_pos"]])
            ...
            if "sent_pos" in encoded_inputs:
                encoded_inputs["sent_pos"] = torch.tensor([encoded_inputs["sent_pos"]])
                encoded_inputs["para_pos"] = torch.tensor([encoded_inputs["para_pos"]])
                encoded_inputs["token_pos"] = torch.tensor([encoded_inputs["token_pos"]])
            if "attention_mask" in encoded_inputs:
    ...
    def build_pos_with_special_tokens(self, text_para_pos,text_sent_pos,text_token_pos,text_pair_para_pos, text_pair_sent_pos, text_pair_token_pos):
