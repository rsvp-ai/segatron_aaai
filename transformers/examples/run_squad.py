...
from transformers import (WEIGHTS_NAME, BertConfig,
                                  SegaBertForQuestionAnswering, BertTokenizer)
...
MODEL_CLASSES = {
    'segabert': (BertConfig, SegaBertForQuestionAnswering, BertTokenizer),
}
    ...
    for _ in train_iterator:
        ...
        for step, batch in enumerate(epoch_iterator):
            ...
            if args.input_type=='para_sent':
                inputs['input_ids'] = {"input_id":batch[0],'para_pos':batch[7],'sent_pos':batch[8]}
            elif args.input_type=='para_sent_token':
                inputs['input_ids'] = {"input_id":batch[0],'para_pos':batch[7],'sent_pos':batch[8],'token_pos':batch[9]}
            elif args.input_type=='para_token':
                inputs['input_ids'] = {"input_id": batch[0], 'para_pos': batch[7], 'token_pos': batch[8]}
            elif args.input_type=='sent_token':
                inputs['input_ids'] = {"input_id": batch[0], 'sent_pos': batch[7], 'token_pos': batch[8]}
...
def evaluate(args, model, tokenizer, prefix=""):
    ...
    for batch in tqdm(eval_dataloader, desc="Evaluating"):
        ...
        with torch.no_grad():
            ...
            if args.input_type=='para_sent':
                inputs['input_ids'] = {"input_id":batch[0],'para_pos':batch[6],'sent_pos':batch[7]}
            elif args.input_type=='para_sent_token':
                inputs['input_ids'] = {"input_id":batch[0],'para_pos':batch[6],'sent_pos':batch[7],'token_pos':batch[8]}
            elif args.input_type=='para_token':
                inputs['input_ids'] = {"input_id":batch[0],'para_pos':batch[6],'token_pos':batch[7]}
            elif args.input_type=='sent_token':
                inputs['input_ids'] = {"input_id":batch[0],'sent_pos':batch[6],'token_pos':batch[7]}
...
def load_and_cache_examples(args, tokenizer, evaluate=False, output_examples=False):
    ...
    cached_features_file = os.path.join(input_dir, 'cached_{}_{}_{}_{}_{}'.format(
        ...
        str(args.max_seq_length),args.input_type,
        'v2' if args.version_2_with_negative else 'v1'),
    )
