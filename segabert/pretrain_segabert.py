...
def get_batch_bert(data_iterator, timers):
...
def get_batch_segabert(data_iterator, timers):
    ''' get_batch subdivides the source data into chunks of
    length args.seq_length. If source is equal to the example
    output of the data loading example, with a seq_length limit
    of 2, we'd get the following two Variables for i = 0:
    ...
    '''
    # Items and their type.
    keys = ['text', 'types', 'para_pos', 'sent_pos','token_pos',
            'is_random', 'mask', 'mask_labels', 'pad_mask']
    datatype = torch.int64
    ...
    tokens = data_b['text'].long()
    types = data_b['types'].long()
    para_poses = data_b['para_pos'].long()
    sent_poses = data_b['sent_pos'].long()
    token_poses = data_b['token_pos'].long()
    next_sentence = data_b['is_random'].long()
    loss_mask = data_b['mask'].float()
    lm_labels = data_b['mask_labels'].long()
    padding_mask = data_b['pad_mask'].byte()

    return tokens, types, para_poses, sent_poses,token_poses, next_sentence, loss_mask, lm_labels, padding_mask
...
def forward_step(data_iterator, model, args, timers):
    ...
    if args.ds_type=='BERT':
        ...
        padding_mask = get_batch_bert(data_iterator, timers)
        timers('batch generator').stop()
        output, nsp = model(tokens, types, 1 - padding_mask,
                            checkpoint_activations=args.checkpoint_activations)
    else:
        tokens, types, para_poses, sent_poses, token_poses, next_sentence, loss_mask, lm_labels, \
            padding_mask = get_batch_segabert(data_iterator, timers)
        timers('batch generator').stop()
        # Forward model.
        input_dict = {'input_id':tokens, 'para_pos':para_poses, 'sent_pos':sent_poses, 'token_pos':token_poses}
        output, nsp = model(input_dict, types, 1-padding_mask,
                            checkpoint_activations=args.checkpoint_activations)
...
def backward_step(optimizer, model, lm_loss, args, timers, zero_grad=True):
    """Backward step."""
    ...
    reduced_losses = lm_loss.view(1) #torch.cat((lm_loss.view(1), nsp_loss.view(1)))
...
def train_step(data_iterator, model, optimizer, lr_scheduler,
    ...
    inner_step = 0
    lm_loss_reduced = 0
    while inner_step < args.gradient_accumulation_steps:
        ...
        zero_grad = inner_step == 0
        lm_loss_reduced += backward_step(optimizer, model, lm_loss,
                                                          args, timers, zero_grad=zero_grad)
...
def evaluate_and_print_results(prefix, data_iterator, model,
    ...
    lm_loss = evaluate(data_iterator, model,
                                 args, timers, verbose)
...
def main():
    ...
    args = get_args(is_sagemaker=is_sagemaker)

    if is_sagemaker:
        args.save = os.environ['SM_MODEL_DIR']
        args.tensorboard_dir = args.save
        args.cache_dir = os.environ['SM_CHANNEL_DATA']
        args.load = os.path.join(args.cache_dir,'load')
    if args.load_pretrained_checkpoint!='none':
        args.pretrained_checkpoint_path = os.path.join(args.cache_dir,
                                                   args.load_pretrained_checkpoint)
    else:
        args.pretrained_checkpoint_path = None
    writer = None
    ...
    iteration = 0
    if args.do_valid:
        prefix = 'the begin of training for val data'
