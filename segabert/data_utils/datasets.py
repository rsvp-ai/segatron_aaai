...
class json_pos_dataset(data.Dataset):
    """
    Class for loading datasets from a json dump.
    Purpose: Useful for loading data for unsupervised modeling or transfer tasks
    Arguments:
    ...
    def __init__(self, path, tokenizer=None, preprocess_fn=None, binarize_sent=False,
        ...
        for j in self.load_json_stream(self.path):
            ...
            sent_pos = j['sent_pos']
            para_pos = j['para_pos']
            self.X.append('_POS_'.join([para_pos, sent_pos,s]))
            self.Y.append(j[label_key])
    ...
    def __getitem__(self, index):
        """gets the index'th string from the dataset"""
        if True:
            raise Exception('Not support json_pos_dataset indexing')
        x = self.X[index]
        if self.tokenizer is not None:
            x = self.tokenizer.EncodeAsIds(x, self.preprocess_fn)
        elif self.preprocess_fn is not None:
...
class bert_sentencepair_pst_dataset(data.Dataset):
    """
    Dataset containing sentencepairs for BERT training. Each index corresponds to a randomly generated sentence pair.
    Arguments:
        ds (Dataset or array-like): data corpus to use for training
    ...
    def __init__(self, ds, max_seq_len=512, mask_lm_prob=.15, max_preds_per_seq=None, short_seq_prob=.01, dataset_size=None, presplit_sentences=False, weighted=True, **kwargs):
        ...
        self.ds.SetTokenizer(None)
        self.max_seq_len = max_seq_len
        self.max_para_pos_len = kwargs['para_pos_length']
        self.max_sent_pos_len = kwargs['sent_pos_length']
        self.max_token_pos_len = kwargs['token_pos_length']
        self.mask_lm_prob = mask_lm_prob
        if max_preds_per_seq is None:
            max_preds_per_seq = math.ceil(max_seq_len*mask_lm_prob /10)*10
        self.max_preds_per_seq = max_preds_per_seq
    ...
    def __getitem__(self, idx):
        ...
        tokensa, tokensb = self.truncate_seq_pair(tokensa, tokensb, self.max_seq_len, rng)
        # join sentence pair, mask, and pad
        tokens, mask, mask_labels, pad_mask = self.create_masked_lm_predictions(tokensa, tokensb,
                                                                                self.mask_lm_prob,
        ...
        sample = {'text': np.array(tokens[0]), 'types': np.array(tokens[1]), 'is_random': int(is_random_next),
                  ...
                  'para_pos': np.array(tokens[2]), 'sent_pos': np.array(tokens[3]), 'token_pos': np.array(tokens[4])}

        while sample['para_pos'].max() >= self.max_para_pos_len:
            if sample['para_pos'][0]==0:
                sample['para_pos'][sample['para_pos']>=self.max_para_pos_len]=self.max_para_pos_len-1
            else:
                sample['para_pos'] = sample['para_pos']-1

        sample['sent_pos'][sample['sent_pos']>=self.max_sent_pos_len]=self.max_sent_pos_len-1
        sample['token_pos'][sample['token_pos'] >= self.max_token_pos_len] = self.max_token_pos_len - 1
        return sample
    ...
    def get_doc(self, idx):
        ...
        if isinstance(rtn, dict):
            rtn = rtn['text']
        rtn = rtn.split('_POS_')[-1]
        return rtn

    def get_para_pos(self, idx):
        """gets text of document corresponding to idx"""
        rtn = self.ds[idx]
        if isinstance(rtn, dict):
            rtn = rtn['para_pos']
        rtn = rtn.split('_POS_')[0]
        return rtn

    def get_sent_pos(self, idx):
        """gets text of document corresponding to idx"""
        rtn = self.ds[idx]
        if isinstance(rtn, dict):
            rtn = rtn['sent_pos']
    ...
    def create_random_sentencepair(self, target_seq_length, rng, np_rng):
        ...
        while curr_len < 1:
            ...
            while doc_a is None:
                ...
                if not doc_a:
                    doc_a = None
            para_pos_a = self.get_para_pos(doc_a_idx).split('\n')
            sent_pos_a = self.get_sent_pos(doc_a_idx).split('\n')
            random_start_a = rng.randint(0, len(doc_a)-1)
            while random_start_a < len(doc_a):
                sentence = doc_a[random_start_a]
                sent_id = sent_pos_a[random_start_a]
                para_id = para_pos_a[random_start_a]
                sentence, sentence_types = self.sentence_tokenize(sentence, 0, random_start_a == 0, random_start_a == len(doc_a))
                curr_strs.append(sentence)
                curr_str_types.append(sentence_types)
                curr_para_poses.append([int(para_id)]*len(sentence))
                curr_sent_poses.append([int(sent_id)]*len(sentence))
                curr_token_poses.append(list(range(len(sentence))))
                curr_len += len(sentence)
                if random_start_a == len(doc_a) - 1 or curr_len >= target_seq_length:
                    break
                random_start_a = (random_start_a+1)
        ...
        if curr_strs:
            ...
            for j in range(num_a):
                tokens_a.extend(curr_strs[j])
                token_types_a.extend(curr_str_types[j])
                para_poses_a.extend(curr_para_poses[j])
                sent_poses_a.extend(curr_sent_poses[j])
                token_poses_a.extend(curr_token_poses[j])
            ...
            for j in range(num_a, len(curr_strs)):
                tokens_b.extend(curr_strs[j])
                token_types_b.extend(curr_str_types[j])
                para_poses_b.extend(curr_para_poses[j])
                sent_poses_b.extend(curr_sent_poses[j])
                token_poses_b.extend(curr_token_poses[j])
        return (tokens_a, token_types_a, para_poses_a, sent_poses_a, token_poses_a), \
               (tokens_b, token_types_b, para_poses_b, sent_poses_b, token_poses_b), is_random_next
    ...
    def truncate_seq_pair(self, a, b, max_seq_len, rng):
        ...
        https://github.com/google-research/bert/blob/master/create_pretraining_data.py#L391
        """
        tokens_a, token_types_a, para_poses_a, sent_poses_a, token_poses_a = a
        tokens_b, token_types_b, para_poses_b, sent_poses_b, token_poses_b = b
        max_num_tokens = self.calc_seq_len(max_seq_len)
        # max_num_tokens = max_seq_len - 3
        while True:
            len_a = len(tokens_a)
            ...
            if len(tokens_a) > len(tokens_b):
                trunc_tokens = tokens_a
                trunc_types = token_types_a
                trunc_para = para_poses_a
                trunc_sent = sent_poses_a
                trunc_token = token_poses_a
            else:
                trunc_tokens = tokens_b
                trunc_types = token_types_b
                trunc_para = para_poses_b
                trunc_sent = sent_poses_b
                trunc_token = token_poses_b
            ...
            if rng.random() < 0.5:
                ...
                trunc_sent.pop()
                trunc_token.pop()
        return (tokens_a, token_types_a, para_poses_a, sent_poses_a, token_poses_a), (tokens_b, token_types_b, para_poses_b, sent_poses_b, token_poses_b)
    ...
    def concat_tokens(self, tokens_a, token_types_a, tokens_b, token_types_b, para_pos_a, sent_pos_a, token_pos_a, para_pos_b, sent_pos_b, token_pos_b):
        tokens = [self.tokenizer.get_command('ENC').Id] + tokens_a + [self.tokenizer.get_command('sep').Id] + tokens_b + [self.tokenizer.get_command('sep').Id]
        token_types = [token_types_a[0]] + token_types_a + [token_types_a[0]] + token_types_b + [token_types_b[0]]
        para_poses = [para_pos_a[0]] + para_pos_a + [para_pos_a[-1]] + para_pos_b + [para_pos_b[-1]]
        sent_poses = [sent_pos_a[0]] + sent_pos_a + [sent_pos_a[-1]] + sent_pos_b + [sent_pos_b[-1]]
        token_poses = [token_pos_a[0]] + token_pos_a + [token_pos_a[-1]+1] + token_pos_b + [token_pos_b[-1]+1]
        return tokens, token_types, para_poses, sent_poses, token_poses
    ...
    def create_masked_lm_predictions(self, a, b, mask_lm_prob, max_preds_per_seq, vocab_words, rng):
        ...
        https://github.com/google-research/bert/blob/master/create_pretraining_data.py#L338
        """
        tokens_a, token_types_a, para_pos_a, sent_pos_a, token_pos_a = a
        tokens_b, token_types_b, para_pos_b, sent_pos_b, token_pos_b = b
        tokens, token_types, para_poses, sent_poses, token_poses = self.concat_tokens(tokens_a, token_types_a,
                                                                         tokens_b, token_types_b,
                                                                         para_pos_a, sent_pos_a, token_pos_a,
                                                                         para_pos_b, sent_pos_b, token_pos_b)
        ...
        output_tokens, pad_mask = self.pad_seq(list(tokens))
        output_types, _ = self.pad_seq(list(token_types))
        output_para_poses,_ = self.pad_seq(list(para_poses))
        output_sent_poses,_ = self.pad_seq(list(sent_poses))
        output_token_poses,_ = self.pad_seq(list(token_poses))
        ...
        return (output_tokens, output_types, output_para_poses, output_sent_poses, output_token_poses), mask, mask_labels, pad_mask