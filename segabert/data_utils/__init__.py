...
from .datasets import json_dataset, csv_dataset, split_ds, ConcatDataset, SplitDataset, bert_sentencepair_dataset, bert_sentencepair_pst_dataset,bert_sentencepair_pt_dataset
...
def make_dataset(path, seq_length, text_key, label_key, lazy=False, process_fn=None, split=[1.],
    ...
    if should_split(split):
        ...
        if 'bert' in ds_type.lower():
            ...
            if 'pst' in ds_type.lower():
                dstype = bert_sentencepair_pst_dataset
                ds = [dstype(d, max_seq_len=seq_length,
                             presplit_sentences=presplit_sentences,
                             para_pos_length=kwargs['para_pos_length'],
                             sent_pos_length=kwargs['sent_pos_length'],
                             token_pos_length=kwargs['token_pos_length']) if d is not None else None for d in ds]
            else:
        ...
        if 'bert' in ds_type.lower():
            ...
            if 'pst' in ds_type.lower():
                dstype = bert_sentencepair_pst_dataset
                ds = dstype(ds, max_seq_len=seq_length, presplit_sentences=presplit_sentences,**kwargs)
            else:
                ds = dstype(ds, max_seq_len=seq_length,
                            presplit_sentences=presplit_sentences,
                            para_pos_length=kwargs['para_pos_length'],
                            sent_pos_length=kwargs['sent_pos_length'],
                            token_pos_length=kwargs['token_pos_length'])
    return ds, tokenizer
