...
class BertConfig(object):
    ...
    def __init__(self,
                 ...
                 max_token_position_embeddings=512,
                 max_para_position_embeddings=50,
                 max_sent_position_embeddings=100,
                 type_vocab_size=2,
        ...
        if isinstance(vocab_size_or_config_json_file, str):
            ...
            self.max_token_position_embeddings = max_token_position_embeddings
            self.max_para_position_embeddings = max_para_position_embeddings
            self.max_sent_position_embeddings = max_sent_position_embeddings
...
class BertEmbeddings(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.token_position_embeddings = mpu.VocabParallelEmbedding(
            config.max_token_position_embeddings, config.hidden_size,
            init_method=normal_init_method(mean=0.0,
                                           std=config.initializer_range))
        self.para_position_embeddings = mpu.VocabParallelEmbedding(
            config.max_para_position_embeddings, config.hidden_size,
            init_method=normal_init_method(mean=0.0,
                                           std=config.initializer_range))
        self.sent_position_embeddings = mpu.VocabParallelEmbedding(
    ...
    def forward(self, input_dict, token_type_ids=None):
        if isinstance(input_dict, dict):
            para_pos_ids,sent_pos_ids = input_dict['para_pos'],input_dict['sent_pos']
            position_ids = input_dict['token_pos']
            input_ids = input_dict['input_id']
            # para_pos_ids = para_pos_ids.long().to(input_ids.device)
            # sent_pos_ids = sent_pos_ids.long().to(input_ids.device)
...
class PreTrainedBertModel(nn.Module):
    ...
    def from_pretrained(cls, pretrained_model_name, state_dict=None, cache_dir=None,
        ...
        # config.max_token_position_embeddings = kwargs['max_token_position_embeddings']
        # config.max_para_position_embeddings = kwargs['max_para_position_embeddings']
        # config.max_sent_position_embeddings = kwargs['max_sent_position_embeddings']
        ...
        for key in state_dict.keys():
            ...
            # if 'token_position_embeddings' in key and kwargs['load_pe_as_tpe']:
            #     new_key = key.replace('token_position_embeddings','position_embeddings')
            if new_key:
                old_keys.append(key)
    ...
    def forward(self, input_ids, token_type_ids=None, attention_mask=None, output_all_encoded_layers=True, checkpoint_activations=False):
        if isinstance(input_ids, dict):
            if attention_mask is None:
                attention_mask = torch.ones_like(input_ids['input_id'])
            if token_type_ids is None:
                token_type_ids = torch.zeros_like(input_ids['input_id'])
        else:
            if attention_mask is None:
