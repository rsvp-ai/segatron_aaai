...
def add_model_config_args(parser,is_sagemaker=False):
    ...
    group.add_argument('--load-pretrained-checkpoint', type=str, default='none',
    ...
    group.add_argument('--max-para-position-embeddings', type=int, default=50,
                       help='maximum number of paragraph position embeddings to use')
    group.add_argument('--max-sent-position-embeddings', type=int, default=100,
                       help='maximum number of sentence position embeddings to use')
    group.add_argument('--max-token-position-embeddings', type=int, default=256,
                       help='maximum number of sentence position embeddings to use')
    group.add_argument('--load-bert-to-segabert', action='store_true',
                       help='whether to load BERT as the initialization of segaBERT ')
...
def add_fp16_config_args(parser,is_sagemaker=False):
    ...
    if is_sagemaker:
        group.add_argument('--fp16', type=boolean_string, default=False,
                           help='Run model in fp16 mode')
        group.add_argument('--fp32-embedding', type=boolean_string, default=False,
                           help='embedding in fp32')
        group.add_argument('--fp32-layernorm', type=boolean_string, default=False,
...
def add_training_args(parser,is_sagemaker=False):
    """Training arguments."""
    ...
    group.add_argument('--gradient-accumulation-steps', type=int, default=1,
                       help='accumulate steps for large batch size training')
    ...
    if is_sagemaker:
        group.add_argument('--resume-dataloader', type=boolean_string, default=False,
...
def add_evaluation_args(parser,is_sagemaker=False):
...
def add_text_generate_args(parser,is_sagemaker=False):
...
def add_data_args(parser,is_sagemaker=False):
    """Train/valid/test data arguments."""

    group = parser.add_argument_group('data', 'data configurations')

    group.add_argument('--ds-type', type=str, default='BERT',
                       help='BERT, BERT_PST, BERT_PT, BERT_ST, GPT2')
    ...
    if is_sagemaker:
        group.add_argument('--lazy-loader', type=boolean_string, default=False,
                           help='whether to lazy read the data set')
        group.add_argument('--loose-json', type=boolean_string, default=False,
                           help='Use loose json (one json-formatted string per '
                           'newline), instead of tight json (data file is one '
                           'json string)')
        group.add_argument('--presplit-sentences', type=boolean_string, default=False,
...
def get_args(is_sagemaker):
    """Parse all the args."""

    parser = argparse.ArgumentParser(description='PyTorch BERT Model')
    parser = add_model_config_args(parser,is_sagemaker)
    parser = add_fp16_config_args(parser,is_sagemaker)
    parser = add_training_args(parser,is_sagemaker)
    parser = add_evaluation_args(parser,is_sagemaker)
    parser = add_text_generate_args(parser,is_sagemaker)
    parser = add_data_args(parser,is_sagemaker)
